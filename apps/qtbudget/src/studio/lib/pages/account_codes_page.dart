...
class _AccountCodesPageState extends State<AccountCodesPage> {
  ...
  void _save() {
    _storage.saveAccountCodes(_codes);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: _codes.length >= 4 ? null : _add,
      ...
      body: _codes.isEmpty
          ...
          : ReorderableListView.builder(
              ...
              itemBuilder: (_, i) {
                final code = _codes[i];
                final color = code.type == AccountType.expense
                    ? Colors.red.shade50
                    : code.type == AccountType.income
                        ? Colors.green.shade50
                        : Colors.blue.shade50;
                final label = switch (code.type) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    subtitle: Text('$label · 层级 ${code.level}'),
                    trailing: Row(
...
class _CodeFormDialogState extends State<_CodeFormDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      title: Text(editing ? '编辑科目' : '新增科目'),
      content: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          TextField(
            controller: _codeCtrl,
            decoration: const InputDecoration(labelText: '科目编码', hintText: '如 A01'),
          ),
          const SizedBox(height: 12),
          TextField(
            controller: _nameCtrl,
            decoration: const InputDecoration(labelText: '科目名称', hintText: '如 办公费用'),
          ),
          const SizedBox(height: 12),
